...
- **플레이 방법**: 앱을 열면 먼저 홈 화면(새로 시작 / 이어하기 / 지난 삶의 기록)이 뜬다. "새로 시작"에서
  ...
  선택지를 고르면 다음 날로 넘어간다. 우측 상단 "상태" 버튼으로 스탯/체력/소지품과 지금까지의 "선택 기록"을
  날짜별로 확인할 수 있고, 그 안에서 "이 삶을 포기한다"를 선택해 삶을 끝낼 수도 있다. 진행 중이던 삶이
  있으면 홈의 "이어하기"로 바로 복귀한다.
...
비용 절감을 위해 매 턴 15%의 확률로만 AI(Claude Haiku 4.5)를 호출하고, 나머지 85%는 처음부터
네트워크 요청 없이 손으로 저작한 오프라인 생성기(시드+낱말뱅크 조합형 라이브러리, 캐릭터/성격/직업/
체력/행운/서사 단계에 따라 가중치를 매겨 선택)로 진행됩니다 — 오프라인 경로는 장애 시 열화판이
아니라 처음부터 1급 경로로 설계되어 있습니다. 자세한 구조는
