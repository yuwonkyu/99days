...
- **AI 호출 비율 15%** (2026-08-06, 사용자 피드백: AI 요금 부담 → 최초 10%로 도입, 같은 날 해커톤 심사 기간 동안 심사위원이 짧은 세션에서도 AI 게임마스터를 실제로 볼 확률을 높이려고 15%로 재조정): `turnService.ts`의 `AI_ATTEMPT_RATE`(0.15)로 매 턴 15% 확률로만 AI 호출을 시도하고, 나머지 85%는 네트워크 요청 자체를 하지 않고 바로 오프라인 생성기로 간다 — Worker 장애 시에만 쓰는 열화판이 아니라 오프라인 생성기 자체를 1급 경로로 취급하는 것으로 방침을 바꿨다. 99일 완주 기준 평균 AI 호출 횟수가 기존 최대 ~100회에서 ~15회로 줄어든다. Worker의 일일 호출 상한(120회/일, `wrangler.toml`)은 이 값과 별개로 여전히 서버 사이드 안전판으로 남아 있다.
...
- 오프라인 폴백([offlineGenerator.ts](../../src/engine/offlineGenerator.ts))도 동일한 `sceneMode`로
  ...
  {timeSkipLabel}이(가) 지나,")를 상황 앞에 붙여 최소한의 연결감을 준다(시드 자체에 이미 "이른 아침,"
...
- **"같은 상황이 반복해서 나온다" 피드백**: `GameState.usedSeedIds`에 캐릭터가 오프라인 폴백에서 이미
  ...
  다시 한 바퀴 순환한다 — 54개 시드가 유한하니 "영원히 반복 없음"은 불가능하지만, 최소한 매 순환마다
...
- [situationSeeds.ts](../../src/data/situationSeeds.ts): `외출/거처` 2모드 × `위기/생계/사교/수수께끼/
  공포/황당/호감(bond)` 7카테고리 = 14버킷에 각 3~7개씩, 총 **54개 시드**를 저작(+위기 전용 5개 별도).
  `bond`는 위험·갈등뿐 아니라 호감·우정·애정 같은 긍정적 관계 감정도 나오도록 추가한 카테고리. 이 중
  대부분은 낱말 뱅크(예: `TIME_MOODS`, `STRANGERS`)를 랜덤으로 섞어 렌더링해, 캐릭터 1명 기준으로도
  **약 290종의 서로 다른 상황 텍스트**가 나온다(2026-08-06 기준, 실측; 이름/직업/성격까지 다르면 실질
  다양성은 더 크다). 선택지도 항상 3택1이 아니라 시드마다 2~4개로 다르게 구성. 버킷 크기는 원래 2~4개로
  고르지 않아, 성격/직업 가중치가 큰 버킷보다 작은 버킷의 개별 시드에 더 쏠리는 문제가 있었음을 실플레이
  제보로 확인(2026-08-06) — 최소 버킷(outdoor/mystery 3, shelter/bond 2 등) 위주로 8개를 보강해 완화.
...
- [endings.ts](../../src/data/endings.ts) / [endingSelector.ts](../../src/engine/endingSelector.ts):
  사망/포기/완주 3종 × 비극/공포/황당/평온/성취 분위기로 나눠 **62개 엔딩 문구**를 저작. 사망 엔딩은
  직전 상황의 장르(공포/위험 등)에 따라 분위기가 가중 랜덤으로 정해진다. 위기(crisisDays) 사망은
  카테고리 키워드 추론에 기대지 않고 `wasCrisisDeath` 플래그로 확정적으로 처리해, peaceful 분위기가
  섞이지 않게 한다(2026-08-06, 위기 사망인데 "잠들듯 평온하게" 나온 제보 대응).
