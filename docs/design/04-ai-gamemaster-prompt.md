...
- **AI 호출 비율 10%** (2026-08-06, 사용자 피드백: AI 요금 부담): `turnService.ts`의 `AI_ATTEMPT_RATE`(0.1)로 매 턴 10% 확률로만 AI 호출을 시도하고, 나머지 90%는 네트워크 요청 자체를 하지 않고 바로 오프라인 생성기로 간다 — Worker 장애 시에만 쓰는 열화판이 아니라 오프라인 생성기 자체를 1급 경로로 취급하는 것으로 방침을 바꿨다. 99일 완주 기준 평균 AI 호출 횟수가 기존 최대 ~100회에서 ~10회로 줄어든다.
